...
# get the tournament name and round name (raw)
...
# result should be two character vactors


rm(tour.1956.raw,round.1956.raw)
# remove unecessary variables

record.1956 <- xpathSApply(data.1956,node.definer.record,xmlValue)
record.1956 <- record.1956[6:(length(record.1956)-3)]
record.1956 <- record.1956.tmp1[nchar(gsub("\\s","",record.1956.tmp1))!=0]
...
dfrm.1956 <- (cbind(home.team=home.team.1956,home.ctry=home.ctry.1956,away.team=away.team.1956,
                    ...
                    round=roundlist.1956))[1:length(match.1956),]
